...
当前配套交付版本为 `0.2.1`，与 Conductor 和嵌入制品一致；后续仅递增 `0.2.x`。
未经用户明确授权不得改到其他版本系列或使用 `0.x.0`。

网页顶部“版本与更新”显示设备实际版本，支持检查更新、阅读说明、确认升级和本机上一版本回退。
任务由设备独立服务执行，网页关闭不会取消；重新打开可继续查询结果。旧固件缺少更新接口时显示
一次性命令行升级提示，不影响原有采集操作。
...
- **远程删除必须显式确认**：设备声明 `session_deletion`，且本次详情响应的 `YLX-Manifest-SHA256`
  与强 `ETag` 一致时，
  页面才允许提交带幂等键的删除请求；连续录制依赖由设备返回并阻止隐式级联删除。
  列表上的快捷删除先读取详情再确认，不依赖可能为空的列表制品校验信息。
...
连接支持校时扩展的 Conductor 后，网页会在设备空闲时自动提供本机日期。
开始录制前先确认校时完成，设备的 NTP 时间优先；正在录制时暂缓调整。
这在设备热点或没有外网的局域网中也能工作，前提是手机/电脑自身日期正确。
设备重启后，保持打开的页面会重新尝试同步。旧固件缺少接口（404）时保持兼容。
