...
import { DeviceApiError, idempotencyKey } from "../api/client";
...
export function FirmwarePanel({ state, onClose }: { state: AppState; onClose: () => void }) {
  ...
  const busy = status?.task?.status === "accepted" || status?.task?.status === "running";
  const idle = state.capture?.snapshot.device_state === "idle" && state.connection === "connected";
  ...
  async function refresh(check = false) {
    ...
    setError(null);
    try {
      const result = await getFirmware(check, check);
      if (alive.current) { setStatus(result); setDisconnected(false); setUnavailable(false); }
    } catch (failure) {
      if (alive.current) {
        setUnavailable(failure instanceof DeviceApiError && failure.status === 404);
        setError(failure instanceof Error ? failure.message : "无法读取更新状态");
      }
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(async () => {
      ...
      try {
        ...
        if (alive.current) { setStatus(result); setDisconnected(false); }
      } catch { if (alive.current) setDisconnected(true); }
  ...
  return <aside class="panel firmware-panel" aria-label="版本与更新">
    ...
    <div class="panel-body">
      ...
      {status?.task ? <section class="detail-section" aria-live="polite">
        <strong>{status.task.status === "succeeded" ? "操作完成" : status.task.status === "failed" ? "操作失败" : "正在更新"}</strong>
        <p>{status.task.message}</p>
        {status.task.status === "succeeded" ? <button type="button" class="chip" onClick={() => window.location.reload()}>刷新页面</button> : null}
      </section> : null}
      ...
      </section> : status?.checked_at && !status.warning ? <p>已是最新版本</p> : null}
      ...
      {confirm ? <section class="detail-section" role="group" aria-label="确认版本操作">
        <p>{confirm.action === "update" ? "确认更新到" : "确认回退到"} v{confirm.version}？服务将自动重启，录制文件保留。</p>
        <div class="chips">
          <button type="button" class="chip" disabled={loading || !idle || busy || disconnected} onClick={() => void submit()}>确认{confirm.action === "update" ? "更新" : "回退"}</button>
